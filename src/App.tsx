...
const App: React.FC = () => {
  ...
  const convertMetrik = (nilaiAwal: number) => {
    ...
    // m ke cm
    if (satuanPertama === 'meter' && satuanKedua === 'sentimeter') return setResult(nilaiAwal / 100)
    // km ke m
    if (satuanPertama === 'kilometer' && satuanKedua === 'meter') return setResult(nilaiAwal * 1000)
    // km ke cm
    if (satuanPertama === 'kilometer' && satuanKedua === 'sentimeter') return setResult(nilaiAwal * 100000)
    // cm ke m
    if (satuanPertama === 'sentimeter' && satuanKedua === 'meter') return setResult(nilaiAwal / 100)
    // cm ke km
    if (satuanPertama === 'sentimeter' && satuanKedua === 'kilometer') return setResult(nilaiAwal / 100000)
    ...
    // C ke K
    if (satuanPertama === 'celcius' && satuanKedua === 'kelvin') return setResult(nilaiAwal + 273.15)
    // F ke C
    if (satuanPertama === 'fahrenheit' && satuanKedua === 'celcius') return setResult((nilaiAwal - 32) * 5 / 9)
    // F ke K
    if (satuanPertama === 'fahrenheit' && satuanKedua === 'kelvin') return setResult((nilaiAwal - 32) * 5 / 9 + 273.15)
    // K ke C
    if (satuanPertama === 'kelvin' && satuanKedua === 'celcius') return setResult(nilaiAwal - 273.15)
    // K ke F
    if (satuanPertama === 'kelvin' && satuanKedua === 'fahrenheit') return setResult((nilaiAwal - 273.15) * 9 / 5 + 32)
      ...
      <IonContent className='ion-padding'>
        <IonGrid>
          ...
          <IonRow>
            <IonCol>
              <IonItem>
                <IonSelect justify="space-between" label="Dari" placeholder="Pilih Satuan" disabled={!metrik} onIonChange={satuanPertamaChangeHandler} ref={satuanPertamaIonSelect}>
            ...
            <IonCol>
              <IonItem>
                <IonSelect justify="space-between" label="Ke" placeholder="Pilih Satuan" disabled={!metrik} onIonChange={satuanKeduaChangeHandler} ref={satuanKeduaIonSelect}>
                  {metrik == 'panjang' && (<SelectPanjang />)}
                  {metrik == 'massa' && (<SelectMassa />)}
                  {metrik == 'waktu' && (<SelectWaktu />)}
                  {metrik == 'arusListrik' && (<SelectArusListrik />)}
          ...
          <IonRow className='ion-margin-top'>
            <IonCol className='ion-margin-top'>
              <IonItem>
                <IonInput type='number' placeholder="Masukkan Nilai Awal" className='ion-text-center' onIonInput={nilaiAwalChangeHandler} ref={nilaiAwalRef}></IonInput>
              </IonItem>
            </IonCol>
          </IonRow>
